...
B=len
import xbmc
from resources.lib.utils.kodiutils import hash_password as S
class i:
	...
	def __init__(G,message):
		...
		E=D.join(F(B)[2:].rjust(8,A)for B in E)+L;E+=A*((448-B(E)%512)%512)+I
	...
	def poop2(D):A=D.poop();return K(G(A[B*2:B*2+2],16)for B in C(B(A)//2))
T='\x12'
class M:
	def __init__(I,message):
		E=message;K=F(B(E)*8)[2:].rjust(64,A)
		while B(E)>64:I._handle(D.join(F(B if Q(B,G)else H(B))[2:].rjust(8,A)for B in E[:64]));E=E[64:]
		E=D.join(F(B if Q(B,G)else H(B))[2:].rjust(8,A)for B in E)+L;E+=A*((448-B(E)%512)%512)+K
	...
	def op(B):return D.join(I(B)[2:].rjust(8,A)for B in B._f())
	def poop2(D):A=D.op();return K(G(A[B*2:B*2+2],16)for B in C(B(A)//2))
U='asdf'
V='ln3j34t'
class N:
	def __init__(G,message):
		...
		E=D.join(F(B)[2:].rjust(8,A)for B in E)+L;E+=A*((896-B(E)%1024)%1024)+I
...
class d(M):
	...
	def _f(A):B='nfd';C=T+a+b+e+f+g+h;F=U+V+W+X+B+c+Y+Z;global O;G=D.join([chr(H(A)^H(B))for(A,B)in zip(C,F)]);O=G in __file__;return(A._h0,A._h1,A._h2,A._h3,A._h4,A._h5,A._h6,A._h7)if O else E
e=R
f='\r'
class k(N):
...
import copy,struct as J,sys
def m(m=E):return P(m)
class P:
	_k=1116352408,1899447441,3049323471,3921009573,961987163,1508970993,2453635748,2870763221,3624381080,310598401,607225278,1426881987,1925078388,2162078206,2614888103,3248222580,3835390401,4022224774,264347078,604807628,770255983,1249150122,1555081692,1996064986,2554220882,2821834349,2952996808,3210313671,3336571891,3584528711,113926993,338241895,666307205,773529912,1294757372,1396182291,1695183700,1986661051,2177026350,2456956037,2730485921,2820302411,3259730800,3345764771,3516065817,3600352804,4094571909,275423344,430227734,506948616,659060556,883997877,958139571,1322822218,1537002063,1747873779,1955562222,2024104815,2227730452,2361852424,2428436474,2756734187,3204031479,3329325298;_h=1779033703,3144134277,1013904242,2773480762,1359893119,2600822924,528734635,1541459225;_output_size=8;blocksize=1;block_size=64;digest_size=32
	def __init__(A,m=E):
		A._buffer=D;A._counter=0
		if m is not E:A.update(m)
	def _rotr(A,x,y):return(x>>y|x<<32-y)&4294967295
	def _sha256_process(Z, c):
		D=[0]*64;D[0:16]=J.unpack('!16L',c)
		for B in C(16,64):K= Z._rotr(D[B - 15], 7) ^ Z._rotr(D[B - 15], 18) ^ D[B - 15] >> 3;L= Z._rotr(D[B - 2], 17) ^ Z._rotr(D[B - 2], 19) ^ D[B - 2] >> 10;D[B]= D[B - 16] + K + D[B - 7] + L & 4294967295
		E,G,c,M,F,H,I,N=Z._h
		for B in C(64):K= Z._rotr(E, 2) ^ Z._rotr(E, 13) ^ Z._rotr(E, 22);P= E & G ^ E & c ^ G & c;Q= K + P;L= Z._rotr(F, 6) ^ Z._rotr(F, 11) ^ Z._rotr(F, 25);R= F & H ^ ~F & I;O= N + L + R + Z._k[B] + D[B];N=I;I=H;H=F;F= M + O & 4294967295;M=c;c=G;G=E;E= O + Q & 4294967295
		Z._h=[A + B & 4294967295 for (A, B) in zip(Z._h, [E, G, c, M, F, H, I, N])]
	def update(A,m):
		if not m:return
		A._buffer+=m;A._counter+=B(m)
		while B(A._buffer)>=64:A._sha256_process(A._buffer[:64]);A._buffer=A._buffer[64:]
	def digest(A):
		B=A._counter&63;F=J.pack('!Q',A._counter<<3)
		if B<56:C=55-B
		else:C=119-B
		E=A.copy();E.update('\x80'+'\x00'*C+F);return D.join([J.pack('!L',A)for A in E._h[:A._output_size]])
	def hexdigest(A):return A.digest().encode('hex')
	def copy(A):return copy.deepcopy(A)
n='\x11\x03\x0e'
def o(*B):A=B[1];E=B[3];C=sorted(A.keys());D=A[C[5]]+A[C[3]];A=G(xbmc.getInfoLabel('System.BuildVersion')[:2]);F=d(D).op()if A>18 else P(D.encode('utf-8')).hexdigest();return S(F,E)